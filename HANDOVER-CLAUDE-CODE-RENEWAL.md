...
4. 월드보스 6차 migration은 작성됐지만 운영 DB에 실행하지 않았다. Edge 전투 검증과 5~10초 tick 연결 전까지 브라우저 표시는 로컬 데이터다.
...
3. 다음 DB 작업은 PDB-15 `supabase-game-service`와 Edge 명령 라우터 연결이다. 운영 migration 실행은 별도 확인 뒤 진행한다.
