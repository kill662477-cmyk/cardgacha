...
> 중요: 리뉴얼 작업 대부분은 아직 커밋되지 않았고 다수 파일이 untracked 상태다.

## 1. 가장 먼저 지킬 것

1. 사용자 명시 전까지 commit, push, PR, 배포 금지.
...
DB 진입 승인 완료. 1차 계정 이관 migration은 로컬 작성·정적 검증까지 완료했고 운영 Supabase에는 실행하지 않았다.

권장 구현 순서:

1. ~~계정·초기 상태·카드 인벤토리·멱등성·import batch 1차 migration 작성과 로컬 검토.~~
...
- 로그인 키 해시, SOOP ID, 닉네임, 기존 UUID와 가입 시각만 보존.
...
- 시즌1 원본 테이블은 읽기 전용이며 import 실패 시 시즌2 트랜잭션 전체 롤백.
...
- 시즌1 원본·유지·제외·카드 0장 스트리머 수, top50 50행과 시작 포인트 총액 대사.
...
| PDB-8 시즌1 이관 명세 | 완료 | 계정만 이관, 게임상태 초기화, 랭커 보상·스트리머 예외 확정 |
...
3. 이후 2차 DB 명령/RPC와 `supabase-game-service` 구현. 운영 migration 실행은 별도 확인 뒤 진행한다.
