...
- 실제 공동 HP, 전체 참가자와 랭킹은 가짜 로컬 값이다.
- 운영 서버 스케줄러와 회차 DB가 없다.
...
PDB-13에서 모험 런·빠른 전투·미니게임 명령을 보완했다. 남은 서버 명령:
...
DB 진입 승인 완료. 계정·브릿지 이관, 서버 카드·밸런스 카탈로그, 명령 기반·편성 RPC, 카드팩·강화·모험·미니게임 RPC, 시즌1 DB 최종 삭제 SQL은 로컬 작성·정적 검증까지 완료했고 운영 Supabase에는 실행하지 않았다.
...
10. `supabase-game-service` 추가 후 UI는 로컬 어댑터와 같은 인터페이스 사용.
...
현재 `gacha_s2_accounts`, `gacha_s2_player_states`, `gacha_s2_player_cards`, `gacha_s2_streamer_bridges`, `gacha_s2_card_catalog`, `gacha_s2_balance_versions`, `gacha_s2_pack_draws`, `gacha_s2_enhancement_results`, `gacha_s2_adventure_runs`, `gacha_s2_minigame_daily`, `gacha_s2_minigame_runs`는 migration과 테스트에 고정됐다. 이후 테이블은 구현 전 명세를 확정할 것.
...
- 보상은 개인 누적 피해와 기존 수령 티어의 차액만 지급.
...
| PDB-9 DB 진입 승인 | 승인 | 1~5차 migration 로컬 작성 완료, 운영 DB 미실행 |
...
4. 월드보스 회차 ID는 로컬에서도 KST 슬롯으로 생성되지만 운영 권위 데이터는 아직 아니다.
5. 브라우저에서 보이는 공동 HP·참가자·랭킹은 실제 멀티플레이 데이터가 아니다.
...
3. 다음 DB 명령은 PDB-14 월드보스 회차·공동 HP·개인 시도·보상 수령부터 구현한다. 이후 `supabase-game-service`를 연결한다. 운영 migration 실행은 별도 확인 뒤 진행한다.
