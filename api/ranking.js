...
const { REST, headers, sbFetch, getUserByKey } = require('../lib/supabase');

const BOT_NICKNAME = '플로우검증봇';

async function loadRankings() {
  return sbFetch(
    `${REST}/gacha_users?select=id,nickname,ranking_score&nickname=not.eq.${encodeURIComponent(BOT_NICKNAME)}` +
      '&order=ranking_score.desc.nullslast,id.asc&limit=10000',
    { headers: headers() }
  );
}

function topFifty(rows) {
  return rows.slice(0, 50).map((row, index) => ({
    rank: index + 1,
    nickname: row.nickname,
    score: row.ranking_score || 0,
  }));
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const rankIndex = rows.findIndex((row) => row.id === user.id);
    if (rankIndex === -1) return sendJson(res, 404, { error: '랭킹 정보를 찾을 수 없습니다' });
    return sendJson(res, 200, {
      rankings: topFifty(rows),
      me: { rank: rankIndex + 1, nickname: user.nickname, score: rows[rankIndex].ranking_score || 0 },
    });
  } catch (e) {
