...
## 4-1. 검증 범위 (정확히 어디까지 했는지)

트랜잭션 롤백으로 **실제 실행**해 확인한 것:

| 대상 | 케이스 |
|---|---|
| `create_guild` | 멱등 재호출 동일 응답·revision 불변, revision +1, 방송인 전용, 중복 생성 차단, revision 충돌, 이름 길이, 감사 로그 |
| `request_join` / `resolve_join` / `kick` / `leave` / `disband` | 신청→승인→가입, 중복 가입 차단, 추방+3일 페널티, 페널티 중 재가입 차단, 자기 추방 차단, 자동승인 즉시 가입, 길드장 탈퇴 차단, 탈퇴+페널티, 해산 시 멤버 행 삭제, 해산 후 재생성 |
| GP·레벨 | 트리거 적립, 비대상 명령 무시, 일일 상한 200, Lv2 승급+정원 40, Lv7 버프값, 무소속 0, 정원 축소 방지 |
| 주간 목표 | 주 시작 월요일, 목표 인원 비례, 1인 999회→상한까지만, 혼자선 달성 불가, 전원 충족 시 달성, 지난주 미집계 |
| 레이드 | 슬롯 5케이스, HP=활동인원×2100만, 유령 회원 미반영, 명단 전원, 멱등, 비슬롯 미생성, 회차 분리 |
| 스냅샷 | 기존 30필드 보존 + `guildBuff` 만 추가 |

**아직 실행 검증하지 않은 RPC** (정적 테스트만 통과):
`update_guild_settings`, `cancel_join_request`, `set_guild_member_role`
— 비교적 단순한 CRUD·권한 분기지만, revision 버그가 "패턴이 같으니 괜찮겠지"로 놓쳤다가
실행에서 잡힌 전례가 있으므로 라이브 반영 전에 한 번 돌려보는 편이 좋다.

## 5. 남은 확인 사항

**시간 의존 경로는 트랜잭션으로 검증할 수 없어 실제 시간대 스모크가 필요하다.**
