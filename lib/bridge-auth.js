...
function cookie(name, value, maxAge) {
  ...
  return `${name}=${encodeURIComponent(value)}; Path=/api/bridge; Max-Age=${maxAge}; HttpOnly; SameSite=Strict${secure}`;
}

function setSession(res, soopId) {
  const nonce = crypto.randomBytes(12).toString('base64url');
  appendCookie(res, cookie(SESSION_COOKIE, issue({ type: 'bridge', nonce, soopId }, SESSION_TTL), SESSION_TTL));
  return nonce;
}
