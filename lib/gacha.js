...
const RACE_BY_MEMBER = require('../data/races.json');
...
let RACE_POOL = null; // race -> rarity -> [card]
function loadCards() {
  if (CARDS) return CARDS;
  const file = path.resolve(__dirname, '..', 'data', 'cards.json');
  CARDS = JSON.parse(fs.readFileSync(file, 'utf8')).map((card) => ({
    ...card,
    race: RACE_BY_MEMBER[card.member] || null,
  }));
  POOL = {};
  RACE_POOL = {};
  for (const r of RARITIES) POOL[r] = [];
  for (const c of CARDS) {
    POOL[c.rarity].push(c);
    if (!c.race) continue;
    if (!RACE_POOL[c.race]) RACE_POOL[c.race] = Object.fromEntries(RARITIES.map((r) => [r, []]));
    RACE_POOL[c.race][c.rarity].push(c);
  }
  return CARDS;
}
...
const PACKS = {
  ...
  luxury: {
    id: 'luxury', name: '프리미엄팩', price: 500, count: 4, guarantee: 'SR',
    odds: { C: 10, U: 15, R: 18, RR: 15, RRR: 12, AR: 9, CHR: 7, HR: 5, SR: 2, SAR: 1.25, UR: 0.75, MUR: 0.35, FUR: 0.15 },
  },
  zerg: {
    id: 'zerg', name: '저그 종족팩', price: 100, count: 3, guarantee: null, race: '저그',
    odds: { C: 49.36, U: 27, R: 14, RR: 6, RRR: 2, AR: 1, CHR: 0.35, HR: 0.15, SR: 0.07, SAR: 0.04, UR: 0.02, MUR: 0.008, FUR: 0.002 },
  },
  terran: {
    id: 'terran', name: '테란 종족팩', price: 100, count: 3, guarantee: null, race: '테란',
    odds: { C: 49.36, U: 27, R: 14, RR: 6, RRR: 2, AR: 1, CHR: 0.35, HR: 0.15, SR: 0.07, SAR: 0.04, UR: 0.02, MUR: 0.008, FUR: 0.002 },
  },
  protoss: {
    id: 'protoss', name: '프로토스 종족팩', price: 100, count: 3, guarantee: null, race: '프로토스',
    odds: { C: 49.36, U: 27, R: 14, RR: 6, RRR: 2, AR: 1, CHR: 0.35, HR: 0.15, SR: 0.07, SAR: 0.04, UR: 0.02, MUR: 0.008, FUR: 0.002 },
  },
};
...
const MEMBER_REWARDS = {'김윤환': 900, '지두두': 600, '치리': 580, '토마토': 750, '비타밍': 510, '주하랑': 490, '임조이': 430, '낭니': 450, '햇살': 450, '소주양': 550, '먼진': 450, '찌킹': 450, '아리송이': 450, '남덕선': 450, '변현제': 260, '김민철': 250, '박수범': 160, '박준오': 150, '사테': 150, '배성흠': 150, '지동원': 150};
...
function pickCardOfRarity(rarity, race = null) {
  loadCards();
  let idx = RANK[rarity];
  while (idx >= 0) {
    const pool = race ? RACE_POOL[race]?.[RARITIES[idx]] : POOL[RARITIES[idx]];
    if (pool && pool.length) {
...
function openPack(packId) {
  ...
  for (let i = 0; i < pack.count; i++) {
    ...
    const card = pickCardOfRarity(rarity, pack.race);
    drawn.push(card);
  }
  return drawn;
}
