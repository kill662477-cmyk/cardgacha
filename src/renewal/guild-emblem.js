...
const EMBLEM_ASSET_VERSION = '202608011945';
...
export const EMBLEM_IMAGES = Object.freeze({
  ...
  s2jjaek: `assets/renewal/guild/emblems/s2jjaek.jpg?v=${EMBLEM_ASSET_VERSION}`,
});

const COVER_EMBLEM_IMAGES = new Set(['s2jjaek']);
...
export function emblemMarkup(key, className = 'guild-emblem-glyph') {
  ...
  if (image) {
    const fitClass = COVER_EMBLEM_IMAGES.has(key) ? ' guild-emblem-image--cover' : '';
    return `<img class="${className} guild-emblem-image${fitClass}" src="${image}" alt="" loading="lazy">`;
  }
