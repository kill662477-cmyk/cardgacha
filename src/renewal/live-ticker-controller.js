...
export function normalizeLiveEvent(raw = {}) {
  ...
  const eventRank = raw.eventRank ?? raw.event_rank ?? null;
  const points = raw.points == null ? null : Number(raw.points);
  const lottoRoundId = String(raw.lottoRoundId ?? raw.lotto_round_id ?? '');
  ...
  const event = {
    ...
    rarity,
    enhancement,
    eventRank: eventRank == null ? null : Number(eventRank),
    points,
    lottoRoundId,
    createdAt,
    createdAtMs: timestamp(createdAt),
  };
  if (!event.nickname || !event.createdAtMs) return null;
  if (
    (eventType === 'lotto_first' && event.eventRank === 1)
    || (eventType === 'lotto_second' && event.eventRank === 2)
  ) {
    return Number.isSafeInteger(points) && points > 0 && lottoRoundId ? event : null;
  }
  if (!event.cardId || !event.member) return null;
...
function semanticKey(event) {
  return [
    event.eventType, event.nickname, event.cardId, event.rarity,
    event.enhancement ?? '', event.lottoRoundId, event.points ?? '',
  ].join('|');
}
...
export function liveEventMarkup(event) {
  if (event.eventType === 'lotto_first' || event.eventType === 'lotto_second') {
    const rank = event.eventRank;
    const color = rank === 1 ? '#f5d76e' : '#d6e0ea';
    return `<span class="live-ticker-item" data-event-type="${event.eventType}" style="--ticker-rarity:${color}"><span class="live-ticker-grade">LOTTO ${rank}등</span><strong>${escapeHtml(event.nickname)}</strong>님이 <em>${Number(event.points).toLocaleString('ko-KR')}P</em>에 당첨됐습니다!</span>`;
  }
...
export function createLiveTickerController({ runtime = null, getNickname = () => '', now = () => Date.now() } = {}) {
  ...
  function render() {
    ...
    const accent = events[0].eventRank === 1
      ? '#f5d76e'
      : events[0].eventRank === 2 ? '#d6e0ea' : RARITIES[events[0].rarity]?.color;
    ticker.style.setProperty('--ticker-accent', accent ?? '#c8f52e');
    const repeated = events.length < 4 ? [...events, ...events, ...events] : events;
    track.innerHTML = repeated.map(liveEventMarkup).join('');
    const characters = repeated.reduce((sum, event) => sum + event.nickname.length + event.member.length + 30, 0);
