-- Relax minigame verification v_max_at to allow for the 15-second grace period
...
declare
  ...
  -- Allow 15 seconds grace period matching expires_at
...
begin
  ...
  for v_action in select value from jsonb_array_elements(p_input_log) loop
    ...
    if v_start < 0 or v_start >= v_count or v_end < 0 or v_end >= v_count or v_start = v_end
      or not v_active[v_start + 1] or not v_active[v_end + 1]
