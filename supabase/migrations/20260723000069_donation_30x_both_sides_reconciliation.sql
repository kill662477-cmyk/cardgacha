...
-- Window: 2026-07-19 00:00 through 2026-07-21 23:59:59 KST.
-- Every recorded amount is eligible, including one-balloon events.
-- A side is excluded when its SOOP ID has no current game account/player state.

begin;

create table if not exists public.gacha_s2_donation_30x_reconciliation_20260719_21 (
...
with eligible_sides as (
  ...
  where event.created_at >= timestamptz '2026-07-18 15:00:00+00'
    and event.created_at < timestamptz '2026-07-21 15:00:00+00'
...
insert into public.gacha_s2_donation_30x_reconciliation_20260719_21 (
  user_id,
  sender_event_count,
...
set points = state.points + reconciliation.points_granted,
    ...
    updated_at = now()
from public.gacha_s2_donation_30x_reconciliation_20260719_21 reconciliation
where state.user_id = reconciliation.user_id
  and reconciliation.points_after is null;

update public.gacha_s2_donation_30x_reconciliation_20260719_21 reconciliation
...
begin
  ...
  select
    ...
    coalesce(sum(greatest(amount * 30 - received_points, 0)), 0)::bigint
  into v_expected_targets, v_expected_grant
  from eligible_sides;

  if (
    select count(*)
    from public.gacha_s2_donation_30x_reconciliation_20260719_21
  ) <> v_expected_targets then
    raise exception 'Donation reconciliation target count mismatch';
  end if;
  ...
  if (
    ...
    raise exception 'Donation reconciliation grant total mismatch';
  end if;
  ...
  if exists (
    ...
    raise exception 'Donation reconciliation amount validation failed';
  end if;
end;
$$;

revoke all on table public.gacha_s2_donation_30x_reconciliation_20260719_21
