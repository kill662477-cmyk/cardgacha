-- 편성 프리셋 저장/적용/삭제 (최대 5개).
--
-- gacha_s2_player_states.formation_presets / active_formation_preset_id 컬럼과
-- 클라이언트 검증(state-schema: 최대 5개, 각 프리셋은 유효한 편성)은 이미 있었는데
-- 값을 쓰는 명령이 없어 항상 비어 있었다. 여기서 그 경로를 붙인다.
--
-- 편성 가능 조건은 gacha_s2_update_formation 과 동일하다: 보유(copies>0) + 비 EX + 도감 등록.
-- 저장 시점뿐 아니라 적용 시점에도 다시 검사한다. 저장 후 카드가 분해되거나 강화 파괴로
-- 사라질 수 있어, 적용만 검사를 건너뛰면 무효한 편성이 들어간다.
...
-- 세 명령이 공유하는 마무리 처리. 응답 생성 + 멱등성 기록 + 감사 로그.
...
begin
  ...
  insert into public.gacha_s2_command_audit (
    ...
    p_user_id, p_idempotency_key, p_command_type, p_request_hash, p_revision - 1, p_revision
  );
  return v_response;
end;
$$;
...
declare
  ...
  v_presets jsonb;
  v_name text := trim(coalesce(p_preset_id, ''));
begin
  if p_user_id is null
    ...
    return public.gacha_s2_command_error(
      p_idempotency_key, 'VALIDATION_FAILED', '프리셋 요청 형식이 올바르지 않습니다.',
      greatest(coalesce(p_expected_revision, 0), 0), null, jsonb_build_object('field', 'preset')
    );
  end if;

  v_request_hash := encode(digest(jsonb_build_object(
    'type', 'saveFormationPreset', 'expectedRevision', p_expected_revision,
    'presetId', v_name, 'formation', to_jsonb(p_formation)
  )::text, 'sha256'), 'hex');
  ...
  -- 새 이름일 때만 개수를 센다. 기존 이름 덮어쓰기는 상한과 무관하다.
  if not (v_presets ? v_name) and (select count(*) from jsonb_object_keys(v_presets)) >= 5 then
    return public.gacha_s2_command_error(
      p_idempotency_key, 'COMMAND_REJECTED', '프리셋은 최대 5개까지 저장할 수 있습니다.',
  ...
  if not public.gacha_s2_formation_preset_cards_ok(p_user_id, p_formation) then
    return public.gacha_s2_command_error(
      p_idempotency_key, 'COMMAND_REJECTED', '도감에 없는 카드나 전투 불가 EX 카드는 저장할 수 없습니다.',
  ...
  return public.gacha_s2_formation_preset_ok(
    p_user_id, p_idempotency_key, 'saveFormationPreset', v_request_hash, v_revision,
    jsonb_build_object('presetId', v_name, 'formation', to_jsonb(p_formation)));
end;
$$;
...
begin
  ...
  v_request_hash := encode(digest(jsonb_build_object(
    'type', 'applyFormationPreset', 'expectedRevision', p_expected_revision, 'presetId', v_name
  )::text, 'sha256'), 'hex');
  ...
  if not (v_presets ? v_name) then
    return public.gacha_s2_command_error(
      p_idempotency_key, 'COMMAND_REJECTED', '해당 프리셋이 없습니다.',
      v_revision, null, jsonb_build_object('field', 'preset'));
  end if;
  ...
  if v_formation is null or cardinality(v_formation) <> 5
    ...
    -- 저장 후 카드를 분해하거나 강화 파괴로 잃으면 여기서 걸린다.
    return public.gacha_s2_command_error(
      p_idempotency_key, 'COMMAND_REJECTED', '프리셋의 카드 중 지금 편성할 수 없는 카드가 있습니다.',
      v_revision, null, jsonb_build_object('field', 'formation'));
  end if;

  update public.gacha_s2_player_states
  set formation = v_formation,
      active_formation_preset_id = v_name,
      revision = revision + 1, updated_at = now()
  where user_id = p_user_id
  returning revision into v_revision;

  return public.gacha_s2_formation_preset_ok(
    p_user_id, p_idempotency_key, 'applyFormationPreset', v_request_hash, v_revision,
    jsonb_build_object('presetId', v_name, 'formation', to_jsonb(v_formation)));
end;
$$;
...
begin
  ...
  v_request_hash := encode(digest(jsonb_build_object(
    'type', 'deleteFormationPreset', 'expectedRevision', p_expected_revision, 'presetId', v_name
  )::text, 'sha256'), 'hex');
  ...
  set formation_presets = v_presets - v_name,
      -- 지운 프리셋이 활성 상태면 함께 비운다. 안 그러면 스냅샷 검증에서
      -- "존재하지 않는 프리셋 ID" 로 걸려 로그인이 막힌다.
  ...
  return public.gacha_s2_formation_preset_ok(
    p_user_id, p_idempotency_key, 'deleteFormationPreset', v_request_hash, v_revision,
    jsonb_build_object('presetId', v_name));
end;
$$;
