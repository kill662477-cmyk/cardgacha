-- SOOP post 202512799 event reward, batch 2.
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conrelid = 'public.gacha_s2_soop_post_202512799_rewards'::regclass
      and conname = 'gacha_s2_soop_post_202512799_rewards_post_soop_key'
  ) then
    alter table public.gacha_s2_soop_post_202512799_rewards
      add constraint gacha_s2_soop_post_202512799_rewards_post_soop_key
      unique (source_post_id, soop_id);
  end if;
end
$constraint$;
...
begin
  ...
  if v_target_count <> 10 or v_unique_soop_count <> 10 then
    raise exception
      'SOOP event batch 2 target mismatch: targets %, unique SOOP IDs %',
      v_target_count,
      v_unique_soop_count;
  end if;

  if v_account_match_count <> 10 or v_account_count <> 10 then
    raise exception
      'SOOP event batch 2 account mismatch: matches %, accounts %',
      v_account_match_count,
      v_account_count;
  end if;

  if v_player_state_count <> 10 then
    raise exception
      'SOOP event batch 2 player-state mismatch: expected 10, actual %',
      v_player_state_count;
  end if;

  if v_existing_reward_count <> 0 or v_existing_mail_count <> 0 then
    raise exception
      'SOOP event batch 2 is no longer new: rewards %, mails %',
      v_existing_reward_count,
      v_existing_mail_count;
  end if;
end
...
create temporary table soop_post_202512799_batch_2 (
...
insert into soop_post_202512799_batch_2 (
  user_id,
  soop_id,
  game_nickname,
  points_before
...
join soop_post_202512799_batch_2 target
...
begin
  ...
  loop
    ...
    if v_mail_id is null then
      raise exception 'SOOP event batch 2 mailbox creation failed';
    end if;
  end loop;
end
$mail$;
...
begin
  ...
  if v_reward_count <> 10 or v_reward_total <> 500000 then
    raise exception
      'SOOP event batch 2 reward verification failed: count %, total %',
      v_reward_count,
      v_reward_total;
  end if;

  if v_mail_count <> 10 then
    raise exception
      'SOOP event batch 2 mailbox verification failed: expected 10, actual %',
      v_mail_count;
  end if;
end
$verify$;
