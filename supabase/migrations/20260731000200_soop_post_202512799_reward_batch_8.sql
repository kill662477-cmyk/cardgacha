-- SOOP post 202512799 event reward, batch 8.
...
create temporary table soop_post_202512799_batch_8_source (
  soop_id text primary key
) on commit drop;

insert into soop_post_202512799_batch_8_source (soop_id)
values
  ('tkv2004'),
  ('yyyyyy93'),
  ('mad4ys');
...
begin
  with account_matches as (
    ...
    from soop_post_202512799_batch_8_source source
    join public.gacha_s2_accounts account
      on lower(trim(account.soop_id)) = source.soop_id
  )
  select
    (select count(*) from soop_post_202512799_batch_8_source),
  ...
  if v_target_count <> 3 then
    raise exception 'SOOP event batch 8 target mismatch: expected 3, actual %', v_target_count;
  end if;

  if v_account_match_count <> 3 or v_account_count <> 3 then
    raise exception
      'SOOP event batch 8 account mismatch: matches %, accounts %',
      v_account_match_count,
      v_account_count;
  end if;

  if v_player_state_count <> 3 then
    raise exception
      'SOOP event batch 8 player-state mismatch: expected 3, actual %',
      v_player_state_count;
  end if;

  if v_existing_reward_count <> v_existing_mail_count
     or v_existing_reward_count not in (0, 3) then
    raise exception
      'SOOP event batch 8 partial prior state: rewards %, mails %',
      v_existing_reward_count,
      v_existing_mail_count;
  end if;
end
...
create temporary table soop_post_202512799_batch_8 (
...
insert into soop_post_202512799_batch_8 (
  user_id,
  soop_id,
  game_nickname,
  points_before
...
create temporary table soop_post_202512799_batch_8_awarded (
  user_id uuid primary key,
  soop_id text not null unique,
  points_before integer not null
) on commit drop;
...
with inserted as (
  ...
  select
    ...
    50000,
    202512799
  from soop_post_202512799_batch_8 target
  on conflict do nothing
  returning user_id, soop_id, points_before
)
insert into soop_post_202512799_batch_8_awarded (
  user_id,
  soop_id,
  points_before
)
...
set points = state.points + 50000,
    ...
    updated_at = now()
from soop_post_202512799_batch_8_awarded awarded
...
join soop_post_202512799_batch_8_awarded awarded
...
begin
  for v_target in
    select user_id
    from soop_post_202512799_batch_8_awarded
    order by soop_id
  loop
    ...
    if v_mail_id is null then
      raise exception 'SOOP event batch 8 mailbox creation failed';
    end if;
  end loop;
end
$mail$;
...
begin
  ...
  join soop_post_202512799_batch_8_source source
  ...
  if v_reward_count <> 3 or v_reward_total <> 150000 then
    raise exception
      'SOOP event batch 8 reward verification failed: count %, total %',
      v_reward_count,
      v_reward_total;
  end if;
  ...
  if v_mail_count <> 3 then
    raise exception
      'SOOP event batch 8 mailbox verification failed: expected 3, actual %',
      v_mail_count;
  end if;
end
$verify$;
