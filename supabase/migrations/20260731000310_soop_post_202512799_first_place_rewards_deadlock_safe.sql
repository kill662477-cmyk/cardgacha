...
-- This replacement for skipped migration 20260731000300 avoids broad table
-- locks. Target player rows are locked in deterministic UUID order instead.
...
select
  ...
  account.nickname,
  case
    when source.event_authenticated or account.is_streamer or source.api_donor
    then 'HIGH'
    else 'STANDARD'
  end,
  source.event_authenticated,
  account.is_streamer,
  source.api_donor,
  state.points,
...
begin
  ...
  if v_target_count <> 2167
    or v_unique_user_count <> 2167
    or v_unique_soop_count <> 2167 then
    raise exception
      'first-place target mismatch: targets %, users %, SOOP IDs %',
      v_target_count,
      v_unique_user_count,
      v_unique_soop_count;
  end if;
...
drop table if exists public.gacha_s2_first_place_reward_diagnostic;
