-- Change API donation ratio to 1:100.
...
begin
  ...
  if v_function not like '%v_points := p_amount * 100%' then
    raise exception 'SOOP donation function verification failed';
  end if;

  select (config #>> '{soopRules,pointsPerBalloon}')::integer
  into v_configured_ratio
  from public.gacha_s2_balance_versions
  where active;

  if v_configured_ratio is distinct from 100 then
    raise exception 'SOOP donation balance config verification failed';
  end if;
end
$verify$;
