-- 1. 모든 탈퇴 페널티 강제 해제 (기록 삭제)
-- 2. 하위 3개 길드(S2 짹 S2, 찌킹단, 섹시테란) 인원에게 50 GP씩 지급 및 길드 GP 갱신

begin;

-- 페널티 해제 (기록 자체를 삭제하여 즉시 가입 가능하도록 처리)
delete from public.gacha_s2_guild_leave_penalties;

do $$
declare
  v_adjustment_prefix text := 'bottom3-bonus-50gp-20260802-';
  v_guild_row record;
  v_member record;
  v_inserted text;
  v_award integer := 50;
