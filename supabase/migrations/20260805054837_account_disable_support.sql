-- 계정 임시 비활성화 지원.
-- disabled_at 이 채워지면 (1) 로그인키 로그인, (2) SOOP 로그인, (3) 이미 로그인된 세션의
-- 게임 커맨드가 모두 막힌다. 되돌리려면 disabled_at 을 null 로 되돌리면 된다.
alter table public.gacha_s2_accounts
  add column if not exists disabled_at timestamptz,
  add column if not exists disabled_reason text;

create index if not exists gacha_s2_accounts_disabled_idx
  on public.gacha_s2_accounts (disabled_at)
  where disabled_at is not null;

-- (3) 게임 커맨 진입점. 모든 커맨드가 auth 세션을 계정으로 바꿀 때 여길 거치므로,
-- 여기서 막으면 이미 발급된 세션을 들고 있어도 아무 것도 할 수 없다.
...
as $function$
  ...
  where auth_user_id = p_auth_user_id and disabled_at is null;
$function$;

-- (1) 로그인키 로그인 차단.
create or replace function public.gacha_s2_bind_auth_session(p_auth_user_id uuid, p_login_key_hash text, p_rate_key text)
returns jsonb
language plpgsql
security definer
set search_path = public, pg_temp
...
begin
  ...
  -- 비활성 계정은 자격증명이 맞아도 로그인시키지 않는다.
  if v_account.disabled_at is not null then
    perform pg_sleep(0.12);
...
-- (2) SOOP 로그인 차단. 교환 코드를 소비하기 전에 막아 코드가 낭비되지 않게 한다.
create or replace function public.gacha_s2_bind_soop_session(p_auth_user_id uuid, p_exchange_code text, p_rate_key text, p_nickname_hint text default null::text)
returns jsonb
language plpgsql
security definer
set search_path = public, pg_temp
...
begin
  ...
  -- 비활성 계정은 교환 코드를 소비하지 않고 즉시 거부한다.
  if v_found and v_account.disabled_at is not null then
    perform pg_sleep(0.12);
    return jsonb_build_object('ok', false, 'code', 'ACCOUNT_DISABLED');
  end if;
...
revoke all on function public.gacha_s2_resolve_auth_account(uuid) from public, anon;
...
grant execute on function public.gacha_s2_resolve_auth_account(uuid) to service_role, authenticated;
