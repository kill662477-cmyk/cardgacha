-- 2026-08-12 월드보스 체력 네 회차 모두 10억씩 상향.
--   17시 265 -> 275억 / 18시 275 -> 285억 / 19시 285 -> 295억 / 20시 295 -> 305억
-- difficultyMultiplier 는 표시용이며 17시 대비 비율에 맞춰 다시 계산했다.
update public.gacha_s2_balance_versions
set config = jsonb_set(jsonb_set(jsonb_set(jsonb_set(jsonb_set(
      config,
      '{worldBossRules,slotTiers,17,maxHp}', '27500000000'::jsonb),
      '{worldBossRules,slotTiers,18,maxHp}', '28500000000'::jsonb),
      '{worldBossRules,slotTiers,19,maxHp}', '29500000000'::jsonb),
      '{worldBossRules,slotTiers,20,maxHp}', '30500000000'::jsonb),
      '{worldBossRules,maxHp}', '27500000000'::jsonb)
where active;

update public.gacha_s2_balance_versions
set config = jsonb_set(jsonb_set(jsonb_set(
      config,
      '{worldBossRules,slotTiers,18,difficultyMultiplier}', '1.036'::jsonb),
      '{worldBossRules,slotTiers,19,difficultyMultiplier}', '1.073'::jsonb),
      '{worldBossRules,slotTiers,20,difficultyMultiplier}', '1.109'::jsonb)
where active;
...
begin
  ...
  if (v_rules->'slotTiers'->'17'->>'maxHp')::bigint <> 27500000000
     or (v_rules->'slotTiers'->'18'->>'maxHp')::bigint <> 28500000000
     or (v_rules->'slotTiers'->'19'->>'maxHp')::bigint <> 29500000000
     or (v_rules->'slotTiers'->'20'->>'maxHp')::bigint <> 30500000000 then
    raise exception 'world boss hp not applied';
  end if;
  if (v_rules->>'maxHp')::bigint <> 27500000000 then
