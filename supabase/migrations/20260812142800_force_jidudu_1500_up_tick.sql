-- One-off operator event: force JDD (지두두) to close 19.43% higher at 2026-08-12 15:00 KST.
-- Pre-seeding the hourly row makes the normal deterministic generator skip only this symbol/hour.
do $$
declare
  v_hour constant timestamptz := timestamptz '2026-08-12 15:00:00+09';
...
begin
  ...
  if v_previous_price is null then
    raise exception 'Cannot force JDD up tick: previous market price is missing';
  end if;

  v_price := greatest(100, round(v_previous_price * 1.1943)::integer);
