-- Emergency disable: keep purchased inventory/data, but remove the race selector from live config
-- and restore the pre-feature player snapshot shape while the connection failure is investigated.
begin;
...
begin
  ...
  if v_config->'directSupportItems' ? 'raceChangeSelector'
    ...
    raise exception 'race change selector emergency disable verification failed';
  end if;
end;
$verify$;
