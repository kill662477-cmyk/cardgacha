-- Keep the x1 underlying cap, but do not freeze leveraged/inverse products at base_price * 10.
-- A practical integer ceiling protects the ledger; new buys are suspended at either safety boundary.
  ...
  while v_cursor <= v_hour loop
    ...
    from (
      select asset.symbol,
             ...
             least(
               case
                 when product.position_type = 'long' and product.multiplier = 1
                   then asset.base_price * 10
                 else 2000000000
               end::numeric,
               greatest(
                 case when product.position_type = 'long' and product.multiplier = 1 then 100 else 1 end::numeric,
                 round(previous_product.price::numeric * (
                   1 + ((current_underlying.price::numeric / previous_underlying.price) - 1)
                     * product.multiplier
                     * case when product.position_type = 'inverse' then -1 else 1 end
                 ))
               )
             )::integer as next_price
...
begin
  if new.side <> 'buy' or new.multiplier < 2 then
    return new;
  end if;

  select asset.base_price * 10
  into v_legacy_cap
  from public.gacha_s2_market_assets asset
  where asset.symbol = new.symbol;

  if new.unit_price <= 1
     or new.unit_price >= 2000000000
     or new.unit_price = v_legacy_cap then
...
declare
  ...
  v_guard_source text;
begin
  v_price_source := pg_get_functiondef(
    'public.gacha_s2_market_ensure_product_prices(timestamptz)'::regprocedure
  );
  v_guard_source := pg_get_functiondef(
    'public.gacha_s2_reject_derivative_buy_at_floor()'::regprocedure
  );
  if v_price_source not like '%else 2000000000%'
     or v_guard_source not like '%new.unit_price >= 2000000000%'
     or v_guard_source not like '%new.unit_price = v_legacy_cap%' then
    raise exception 'MARKET_DERIVATIVE_UPPER_CAP_FIX_GUARD_FAILED';
  end if;
end;
$$;
