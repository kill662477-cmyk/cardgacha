-- Card Gacha Season 2: quick battle now resets on the same 4-hour rolling
-- window as adventure runs, instead of a calendar-day (KST midnight) reset.
-- Quick battle keeps its own independent windowStartedAt/count -- it does not
-- merge with the adventure_runs pool it already shares consumption with.
--
-- Backfills existing gacha_s2_player_states.quick_battle rows (still the old
-- {date,count} shape) to {windowStartedAt,count}. Safe: every row currently
-- has count = 0 (confirmed via a manual reset of the handful of non-zero
-- accounts before this migration), so this is a pure shape normalization,
-- not a player-facing reward or limit change.

alter table public.gacha_s2_player_states
  alter column quick_battle set default jsonb_build_object('windowStartedAt', 0, 'count', 0);

update public.gacha_s2_player_states
set quick_battle = jsonb_build_object('windowStartedAt', 0, 'count', 0)
where not (quick_battle ? 'windowStartedAt');
...
begin
  ...
  -- 빠른 전투 횟수: 달력 날짜가 아니라 모험 런과 동일한 4시간 롤링 윈도우로 초기화.
