-- Card Gacha Season 2: include enhancement level in each leaderboard entry's
-- formation, so the ranker-deck modal can show real star counts instead of
-- always 0 (migration 021 only sent bare card ids).
--
-- `formation` entries become {cardId, enhancement} objects instead of plain
-- card_id strings. Enhancement is looked up per-card from gacha_s2_player_cards
-- (only for the top-50 rows actually returned, via a correlated subquery -- at
-- most 50*5 = 250 lookups on an indexed (user_id, card_id) primary key).
