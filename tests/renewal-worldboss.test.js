...
// balance-tune: 서버 자동딜 폐지 -> 처치는 순수 참가자 합산딜(여기선 43.8M) vs maxHp(110억) 비교.
// 43.8M << 110억이라 이 소규모 참여로는 처치 실패(참여 부족 시 실패도 발생하는 것이 의도된 설계).
assert.equal(resultSnapshot.defeated, false, '43.8M pooled damage falls far short of the 110억 maxHp target');
