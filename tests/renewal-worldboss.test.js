...
// balance-tune: 서버 자동딜 폐지 -> 처치는 순수 참가자 합산딜(여기선 34.4M) vs maxHp(65억) 비교.
// 34.4M << 65억이라 이 소규모 참여로는 처치 실패(참여 부족 시 실패도 발생하는 것이 의도된 설계).
assert.equal(resultSnapshot.defeated, false, '34.4M pooled damage falls far short of the 65억 maxHp target');
...
// balance-tune: 서버 자동딜 폐지 -> 처치 기준은 순수 참가자 합산딜 vs maxHp(17:00 슬롯 65억)의 경계값.
const successBoundary = { ...progress, attempts: 1, totalDamage: 6_500_000_000 };
const belowSuccessBoundary = { ...progress, attempts: 1, totalDamage: 6_499_999_999 };
