...
// balance-tune: 서버 자동딜 폐지 -> 처치는 순수 참가자 합산딜(여기선 34.4M) vs maxHp(60억) 비교.
...
assert.equal(getWorldBossTier(atOpen.slot.id).maxHp, 6_000_000_000, '17:00 baseline HP = 60억 (server DPS removed)');
assert.equal(getWorldBossTier('noise-zero-20260717-18').maxHp, 6_500_000_000);
assert.equal(getWorldBossTier('noise-zero-20260717-19').maxHp, 7_000_000_000);
assert.equal(getWorldBossTier('noise-zero-20260717-20').maxHp, 7_500_000_000);
assert.deepEqual(
...
// balance-tune: 서버 자동딜 폐지 -> 처치 기준은 순수 참가자 합산딜 vs maxHp(17:00 슬롯 60억)의 경계값.
const successBoundary = { ...progress, attempts: 1, totalDamage: 6_000_000_000 };
const belowSuccessBoundary = { ...progress, attempts: 1, totalDamage: 5_999_999_999 };
